#!/bin/sh
# run.sh -- build (if needed) and launch Battleship in the FujiNet-patched
...
# last built.
if [ ! -f battleship.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer battleship.rom)" ]; then
    echo "Building battleship.rom..."
    make
fi
